...
\cpnote{Nota que el parrafo tiene dos partes no conexas. Separa de alguna manera. 
Quizá funcione un simple punto y aparte. El titulo de la seccion no me gusta, pues
entonces uno pensaría que para que mas secciones (si el titulo del caputolo es resultados
y el capitulo de la seccion es resultados, tonces para que mas secciones)}
\janote{El punto y aparte nos hace la tarea :P}
\janote{Ya que ese título de sección fue el que me aprobaron en el 
protocolo de Tesis no lo podemos cambiar :S}
...
que contamos con una supercomputadora
%computadora promedio de un estudiante guatemalteco 
%de física que tiene memoria RAM ilimitada y 
que puede analizar la completa positividad a una tasa de $10^9$ operaciones PCE de 
...
componentes de Pauli.
\cpnote{Usa los signos de excalmación 
formales, es decir solo uno a cada lado. Por otro lado, creo qeupuedes hacer el calculo 
asumiendo una supercomputadora y no cambiaría mucho. Quizá en vez de 1/5 de la edad 
del universo, tengas muchas veces la edad de la tierra o algun otro dato 
asi diverido.}. 
\cpnote{La ram realmente no es un factor aca. Creo qe 
podrías organizarte y ocupar una cantidad de ram muy razonable. Si no es asi, 
...
%componentes de Pauli. \cpnote{quizá debemos quitar este parrafo y la
%correspondiente figra}
%\begin{figure}
...
operaciones PCE hasta de 3 qubits\cpnote{Representar represetnar}\janote{jeje ya}. En este punto, esta afirmación puede 
...
En esta sección discutiremos con detalle el análisis de los resultados 
de los canales PCE de 2 y 3 qubits presentados en las secciones anteriores. 
Vamos a hablar sobre las propiedades que nuestros resultados muestran 
que una operación PCE debe tener para ser una operación completamente 
positiva y, por lo tanto, ser un canal cuántico\cpnote{no entiendo esta frase}. 
Utilizaremos de forma recurrente la representación de las figuras PCE 
a lo largo de esta sección para hacer más intuitiva la discusión.
...
\begin{enumerate}
	...
	de qubits de un sistema. Para los canales PCE de 2 qubits 
	esto puede verse como transposiciones de las figuras PCE 
	de la \Fref{fig:2qubits_PCEChannels_figs}. A continuación,
	mostramos cuál es el canal PCE equivalente al primer elemento 
	de C${}_4^3$ en la \Fref{fig:C_4^3}, mediante intercambio 	de partículas.
	\begin{center}
	...
	en la \Fref{fig:2qubits_PCEChannels_figs}. En general, la permutación de los 
	...
	ejemplo de un canal PCE equivalente al primer elemento de C${}_8^2$
...
\cpnote{Siento que acá frenas en seco. Tienes dos pequeños resultados que leugo no 
capitalizas. Qeu hacemos con esas permutacion? Puedes mostrar todos los elementos que
caen en una clase? Para que nos sirve este resultado?}
...
En general, nuestros resultados numéricos de los canales PCE 
de 2 y 3 qubits exhiben las siguientes propiedades de los canales PCE:
\cpnote{PCE y PCE repetido. creo que la calidad de la redaccion está bajando.}
\begin{enumerate}
	...
	como función del logaritmo de base 2 del número $2^k$ de componentes de Pauli 
	invariantes, es simétrico respecto a $k=n$\cpnote{No se entiende. Que tal si le das una revisada a la redaccion cuando andes descansado a lo que queda de esta seccion?}. En la \Fref{fig:mirroring} mostramos 
	las gráficas del número de canales PCE de 1, 2 y 3 qubits 
	en función de $\log_2(2^k)$. Nótese la simetría respecto a 
	...
	que los puntos en azul de la gráfica de la \Fref{fig:mirroring_3qubits}, que no representan resultados nuestros,
	también existen. De este modo la gráfica sería simétrica respecto a $k=3$, como
	la regla espejo establece.
	\begin{figure} % {{{
	\centering
	\begin{subfigure}[b]{0.48\textwidth}
		\centering
...
La acción de un canal PCE sobre cualquier partición del sistema debe 
ser otro canal PCE. Esta propiedad no la incluimos dentro del listado que 
recién describimos porque es una consecuencia general 
de los canales cuánticos, no específicamente de los canales PCE. Esto puede 
visualizarse en las figuras PCE de la siguiente forma. En todas las figuras PCE
de la \Fref{fig:2qubits_PCEChannels_figs} deben reconocerse canales PCE de 1 qubit 
en la primera fila y en la primera columna, que representan la acción del 
canal PCE sobre cada qubit individualmente. Por ejemplo, la operación PCE 
...
los planos con cubos rojos y azuels en 3 de las 6 caras del cubo 
(ver \Fref{fig:PCE_figs})
deben representar
...
de los canales PCE de 2 qubits (\Fref{fig:2qubits_PCEChannels_figs}) 
y de las propiedades de los canales PCE que enlistamos antes,
...
\begin{enumerate}
	...
	$\tau_{ij}$, igual a 1. Según la clasificación en clases de equivalencia, 
...
El objetivo de discutir las reglas empíricas para los canales PCE de 2 qubits 
es proporcionar intuición, a partir de las figuras PCE, de que los canales PCE
obedecen alguna estructura matemática. 
...
para analizar las características en común de los canales PCE. 
...
de elementos de una base local. Nuestros resultados numéricos
muestran que los canales PCE obedecen dos reglas. Por un lado, la regla $2^k$ para el número de 
componentes de Pauli que dejan invariantes. Por otro lado, la regla espejo sobre 
la simetría del número de canales PCE, en función del logaritmo de base 2
del número $2^k$ de componentes de Pauli invariantes, respecto a $k=n$. 
Estas propiedades, junto con la discusión de las reglas empíricas para construir canales PCE 
de 2 qubits en esta sección, son prueba de una estructura matemática 
de los canales PCE.  
