...
\cpnote{ortonormal} \janote{Listo}
...
\cpnote{Anotación tecnica importante: hay diferencia 
...
en el documento e incluye ``ortonormal'' cuanod sea apropiado.} 
\janote{Sí veo el problema de no utilizar el apellido `ortonormal'. Luego de 
que enuncio el uso de una `base' utilizo 
la relación de ortonomalidad ($\braket{\psi_i}{\psi_k}=\delta_{ij}$)
en mis deducciones. Esta relación es propia de una base
ortonormal. Para una base (a secas) eso no tiene por qué cumplirse.
De hecho, en general no se cumple.}
...
se llama de la misma manera} \janote{Tienes razón. Dicho eso, hasta hace
bulto esta línea entonces. Yo la quitaría y que el álgebra continúe de 
corrido.}
...
es tu informe!}
\janote{Me parece bien tu sugerencia. Funcionan los nuevos kets?}
. Es sencillo probar
que, ya que la traza es una operación lineal, la traza parcial también 
lo es. 
\janote{En vista de lo que discuto a tu último comentario agrego 
...
discutimos.}
\janote{Voy a asumir que te refieres al uso de la traza parcial como 
\textbf{única}
operación para 
definir al operador de densidad reducido. Si es así, estoy confundido 
porque la ecuación \eqref{eq:reducedRho-def1} ya lo evidencia. Sin embargo,
no lo menciono por ninguna parte en palabras. Por ello agrego un enunciado
más al párrafo anterior a estos comentarios.}
